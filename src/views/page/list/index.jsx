...
// ** Store & Actions
import { getPageFiltered, getPages, getPageSearchFiltered } from "../store"
    ...
    <div className="invoice-list-table-header w-100 py-2">
      <Row>
        <Col lg="6" className="d-flex align-items-center px-0 px-lg-1">
          ...
          <Button tag={Link} to="/page/add" color="primary">
            Add Page
          </Button>
        </Col>
        <Col
...
const PagesList = () => {
  ...
  const store = useSelector((state) => state.page)
  const { get: getPageList } = useFetch()
  ...
  const handleFilter = (val) => {
    ...
    const filteredData = store.allData.filter((page) => {
      /* eslint-disable operator-linebreak, implicit-arrow-linebreak */
      return (
        page.title.toLowerCase().includes(queryLowered) ||
        page.intro.toLowerCase().includes(queryLowered) ||
        page.content.toLowerCase().includes(queryLowered) ||
        String(page.id).toLowerCase().includes(queryLowered)
      )
    })
    dispatch(
      getPageSearchFiltered({
        data: filteredData,

...
const ReportsList = () => {
  ...
  const { get: getReportList } = useFetch()
  ...
  useEffect(() => {
    getReportList("/post/report").then((res) => {
      dispatch(
        getReports({
          q: value,
          page: currentPage,
  ...
  const handleFilter = (val) => {
    ...
    const filteredData = store.allData.filter((error) => {
      ...
      return JSON.stringify(error).toLowerCase().includes(queryLowered)
    })
    dispatch(
      getReportSearchFiltered({
        data: filteredData,

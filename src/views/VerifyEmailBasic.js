...
const VerifyEmailBasic = () => {
  ...
  const emailResend = event => {
    event.preventDefault()
    console.log(auth.currentUser.email)
    sendEmailVerification(auth.currentUser)
      .then(() => {
        setDisabled(true)
        toast.info(t('messages.new_email_sent'))
      })
      .catch(err => {
      ...
      <div className='auth-inner my-2'>
        <Card className='mb-0'>
          <CardBody>
            ...
            <Button block tag="a" href='/login' color='primary'>
              Continue
            </Button>
            <p className='text-center mt-2'>
              <span>Didn't receive an email? </span>
              <a href disabled={disabled} onClick={emailResend} className='fw-bolder'>
                <span>Resend</span>
              </a>
            </p>
